...
export default class LiveWebsiteClient implements WebsiteClient {
  ...
  constructor() {
    this.masterApiBaseUrl =
      process.env.MASTER_API_BASE_URL || "http://localhost:8000/api";
  ...
  async getGame(gameId: string): Promise<StoredGameData | null> {
    try {
      const response = await this.request.get(
        `${this.masterApiBaseUrl}/game/${gameId}`,
      );
  ...
  async getUser(userId: string): Promise<StoredUserData | null> {
    try {
      const response = await this.request.get(
        `${this.masterApiBaseUrl}/user/${userId}`,
      );
  ...
  async saveGame(
    ...
    serializedGame: any,
    viewOfGame: any,
    players: { userId: string; data: object }[],
    state: string,
    version: string,
  ...
  async clearChatRoom(roomId: string): Promise<void> {
    await httpDelete(`${this.masterApiBaseUrl}/clearChatRoom/${roomId}`).auth(
      this.masterApiUsername,
      this.masterApiPassword,
      true,
    );
  }
  ...
  async isGameCancelled(gameId: string): Promise<boolean> {
    try {
      const response = await this.request.get(
        `${this.masterApiBaseUrl}/game/${gameId}/isCancelled`,
      );
