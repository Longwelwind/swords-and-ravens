 ...
 */
export default abstract class WildlingCardEffectInTurnOrderGameState<
  C extends GameState<any, any>,
> extends GameState<WildlingsAttackGameState, C> {
  get game(): Game {
  ...
  proceedNextHouse(previousHouse: House | null): void {
    ...
    if (consequence.victoryConditionsFulfilled) {
      ...
      this.onTakeControlOfEnemyPortGameStateRequired(
        consequence.takeOverPort,
        previousHouse,
      );
      return;
    }
  ...
  onTakeControlOfEnemyPortGameStateRequired(
    _takeOver: TakeOverPort,
    _previousHouse: House,
  ): void {
    throw new Error(
      "If necessary, child states must override onTakeControlOfEnemyPortGameStateRequired!",
    );
  }
