...
export default class MusteringGameState extends GameState<
  ParentGameState,
...
> {
  ...
  firstStart(onlyAtCastles = false): void {
    this.musteringType = onlyAtCastles
  ...
  proceedNextHouse(house: House | null = null): void {
    if (
      this.game.ironThroneTrack[this.game.ironThroneTrack.length - 1] == house
    ) {
  ...
  static deserializeFromServer(
    ...
    data: SerializedMusteringGameState,
  ): MusteringGameState {
    const mustering = new MusteringGameState(parent);

    mustering.musteringType = data.musteringType;
    mustering.childGameState = mustering.deserializeChildGameState(
      data.childGameState,
    );
  ...
  deserializeChildGameState(
    data: SerializedMusteringGameState["childGameState"],
  ): PlayerMusteringGameState {
