...
import ChooseRegionForLoyaltyTokenGameState, {
  SerializedChooseRegionForLoyaltyTokenGameState,
} from "./choose-region-for-loyalty-token-game-state/ChooseRegionForLoyaltyTokenGameState";
import {
  domesticDisputes,
  emptyPromises,
  fireMadeFlesh,
...
import ChooseMultipleRegionsForLoyaltyTokenGameState, {
  SerializedChooseMultipleRegionsForLoyaltyTokenGameState,
} from "./choose-multiple-regions-for-loyalty-token-game-state/ChooseMultipleRegionsForLoyaltyTokenGameState";
import FireMadeFleshGameState, {
  SerializedFireMadeFleshGameState,
} from "./fire-made-flesh-game-state/FireMadeFleshGameState";
import PlayingWithFireGameState, {
  SerializedPlayingWithFireGameState,
} from "./playing-with-fire-game-state/PlayingWithFireGameState";
import TheLongPlanGameState, {
  SerializedTheLongPlanGameState,
} from "./the-long-plan-game-state/TheLongPlanGameState";
import MoveLoyaltyTokensGameState, {
  SerializedMoveLoyaltyTokensGameState,
} from "./move-loyalty-tokens-game-state/MoveLoyaltyTokensGameState";

export default class WesterosDeck4GameState extends GameState<
  WesterosGameState,
  | ChooseRegionForLoyaltyTokenGameState
  | ChooseMultipleRegionsForLoyaltyTokenGameState
  | FireMadeFleshGameState
  | PlayingWithFireGameState
  | TheLongPlanGameState
...
> {
  ...
  firstStart(type: WesterosCardType): void {
    const isVassal =
      this.game.targaryen && this.ingame.isVassalHouse(this.game.targaryen);
    if (
      isVassal ||
      this.ingame.isHouseDefeated(this.game.targaryen) ||
      !this.game.targaryen
    ) {
      this.ingame.log({
