...
import TyrionLannisterAbilityGameState from "../../action-game-state/resolve-march-order-game-state/combat-game-state/cancel-house-card-abilities-game-state/tyrion-lannister-ability-game-state/TyrionLannisterAbilityGameState";
...
export default class TyrionLannisterHouseCardAbility extends HouseCardAbility {
  cancel(
    ...
    cancelResolutionState.childGameState
      .setChildGameState(
        new TyrionLannisterAbilityGameState(
          cancelResolutionState.childGameState,
        ),
      )
      .firstStart(house);
