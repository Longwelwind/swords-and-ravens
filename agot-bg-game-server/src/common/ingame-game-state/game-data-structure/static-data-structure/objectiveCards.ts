...
export const backdoorPolitics = new ObjectiveCard(
    ...
    (house, ingame) => {
        const victoryPointsPerHouse = new BetterMap(
            ingame.game.nonVassalHouses.map(h => [h, ingame.game.getVictoryPoints(h)]));
...
export const landGrab = new ObjectiveCard(
    ...
    (house, ingame) => {
        const landAreasPerHouse = new BetterMap(
            ingame.game.nonVassalHouses.map(h => [h, ingame.game.getTotalControlledLandRegions(h)]));
...
export const extendYourReach = new ObjectiveCard(
    ...
    (house, ingame) => {
        const seaAreasPerHouse = new BetterMap(
            ingame.game.nonVassalHouses.map(h => [h, ingame.world.regions.values.filter(r => r.type == sea && r.getController() == h).length]));
...
export const navalSuperiority = new ObjectiveCard(
    ...
    (house, ingame) => {
        const shipCountPerHouse = new BetterMap(
            ingame.game.nonVassalHouses.map(h => [h, ingame.world.getUnitsOfHouse(h).filter(u => u.type == ship).length]));
...
export const mercantileVentures = new ObjectiveCard(
    ...
    (house, ingame) => {
        const controlledPortsPerHouse = new BetterMap(
            ingame.game.nonVassalHouses.map(h => [h, ingame.world.regions.values.filter(r => r.type == port && r.getController() == h).length]));
...
export const aStalwartPosition = new ObjectiveCard(
    ...
    (house, ingame) => {
        const controlledStrongholds = new BetterMap(
            ingame.game.nonVassalHouses.map(h => [h, ingame.world.regions.values.filter(r => r.castleLevel == 2 && r.getController() == h).length]));
...
export const spreadingTheWealth = new ObjectiveCard(
    ...
    (house, ingame) => {
        const powerTokensOnBoard = new BetterMap(
            ingame.game.nonVassalHouses.map(h => [h, ingame.world.regions.values.filter(r => r.controlPowerToken == h).length]));
...
export const friendlyConfines = new ObjectiveCard(
    ...
    (house, ingame) => {
        const controlledCastles = new BetterMap(
            ingame.game.nonVassalHouses.map(h => [h, ingame.world.regions.values.filter(r => r.castleLevel == 1 && r.getController() == h).length]));
...
export const arrynSpecialObjective = new SpecialObjectiveCard(
    ...
    (house, ingame) => {
        const powerTokensPerHouse = new BetterMap(
            ingame.game.nonVassalHouses.map(h => [h, h.powerTokens]));
