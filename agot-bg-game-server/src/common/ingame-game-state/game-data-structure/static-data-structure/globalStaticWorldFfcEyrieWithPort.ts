import * as ffcEyrieWithPort from "../../../../../data/ffc-eyrie-with-port.json";
...
const regionsLayer = ffcEyrieWithPort.layers.find(l => l.name == "Regions");
if (!regionsLayer) {
    throw new Error("No layer named Regions in map file");
}

const bordersLayer = ffcEyrieWithPort.layers.find(l => l.name == "Borders");
if (!bordersLayer) {
    throw new Error("No layer named Borders in map file");
}

const unitSlotsLayer = ffcEyrieWithPort.layers.find(l => l.name == "Unit Slots");
if (!unitSlotsLayer) {
    throw new Error("No layer named Unit Slots in map file");
}

const orderSlotsLayer = ffcEyrieWithPort.layers.find(l => l.name == "Order Slots");
if (!orderSlotsLayer) {
    throw new Error("No layer named Order Slots in map file");
}

const powerTokenSlotsLayer = ffcEyrieWithPort.layers.find(l => l.name == "Power Token Slots");
if (!powerTokenSlotsLayer) {
    throw new Error("No layer named Power Token Slots in map file");
}

const improvementSlotsLayer = ffcEyrieWithPort.layers.find(l => l.name == "Improvement Slots");
if (!improvementSlotsLayer) {
    throw new Error("No layer named Improvement Slots in map file");
}

const castleSlotsLayer = ffcEyrieWithPort.layers.find(l => l.name == "Castle Slots");
if (!castleSlotsLayer) {
...
const staticWorldFfcEyrieWithPort = new StaticWorld(regions, borders);

export default staticWorldFfcEyrieWithPort;
