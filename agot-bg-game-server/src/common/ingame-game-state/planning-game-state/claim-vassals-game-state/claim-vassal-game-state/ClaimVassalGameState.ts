...
export default class ClaimVassalGameState extends GameState<ClaimVassalsGameState> {
  ...
  onPlayerMessage(player: Player, message: ClientMessage): void {
    if (message.type == "claim-vassal") {
      ...
      const claimedVassals = message.houses.map((hid) =>
        this.ingame.game.houses.get(hid)
      );
      ...
      this.parentGameState.passedVassalsCount =
        this.count - claimedVassals.length;
  ...
  static deserializeFromServer(
    ...
    claimVassal.claimableVassals = data.claimableVassals.map((hid) =>
      claimVassals.game.houses.get(hid)
    );
