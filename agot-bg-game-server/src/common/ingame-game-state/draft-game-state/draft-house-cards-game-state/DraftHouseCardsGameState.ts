...
export default class DraftHouseCardsGameState extends GameState<
  DraftGameState,
  SelectHouseCardGameState<DraftHouseCardsGameState> | SimpleChoiceGameState
> {
  houses: House[];
  draftOrder: number[][];
  ...
  proceedNextHouse(): void {
    ...
    if (
      houseHasPositionOnAllInfluenceTracks ||
      houseToResolve == this.game.targaryen
    ) {
  ...
  getNextHouse(): House | null {
    ...
    const housesWithoutTargaryen = this.game.targaryen
      ? _.without(this.houses, this.game.targaryen)
      : this.houses;
    if (
      this.houses.every((h) => h.houseCards.size == 7) &&
    ...
    return this.houses[
      this.draftOrder[this.currentRowIndex][this.currentColumnIndex]
    ];
  }
  ...
  static deserializeFromServer(
    ...
    draftHouseCardsGameState.childGameState =
      draftHouseCardsGameState.deserializeChildGameState(data.childGameState);
