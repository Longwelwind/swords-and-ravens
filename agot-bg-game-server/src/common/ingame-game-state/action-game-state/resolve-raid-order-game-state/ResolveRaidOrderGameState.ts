...
} from "./resolve-single-raid-order-game-state/ResolveSingleRaidOrderGameState";
...
export default class ResolveRaidOrderGameState extends GameState<
  ActionGameState,
...
> {
  ...
  firstStart(): void {
    if (
      this.actionGameState.planningRestrictions.some((pr) => pr == noRaidOrder)
    ) {
  ...
  getNextHouseToResolveRaidOrder(
    ...
    for (let i = 0; i < this.game.houses.size; i++) {
      ...
      currentHouseToCheck =
        this.ingameGameState.getNextInTurnOrder(currentHouseToCheck);
    }
  ...
  getRegionsWithRaidOrderOfHouse(
    ...
    return this.actionGameState
      ...
      .filter(([r, _ot]) => !this.resolvedRaidSupportOrderRegions.includes(r));
  }
  ...
  static deserializeFromServer(
    ...
    resolveRaidOrder.childGameState =
