...
import ResolveSpymasterGameState, {
  SerializedResolveSpymasterGameState,
} from "./resolve-spymaster-game-state/ResolveSpymasterGameState";
import WesterosCard from "../../../../../ingame-game-state/game-data-structure/westeros-card/WesterosCard";
import _ from "lodash";

export default class SpymasterGameState extends GameState<
...
> {
  ...
  getTopTwoCardsOfDeck(deckId: number): WesterosCard[] {
    return _.take(
      this.game.westerosDecks[deckId].filter((wc) => !wc.discarded),
      2
    );
  }
  ...
  onSimpleChoiceGameStateEnd(choice: number): void {
    ...
    this.setChildGameState(new ResolveSpymasterGameState(this)).firstStart(
      house,
      this.choices.values[choice],
      this.getTopTwoCardsOfDeck(this.choices.values[choice])
    );
  }

  onResolveSpymasterFinish(
    house: House,
    westerosDeckId: number,
    westerosCardsForTopOfDeck: WesterosCard[],
    westerosCardsForBottomOfDeck: WesterosCard[]
  ): void {
    ...
    const concatenatedForCheck = westerosCardsForTopOfDeck.concat(
      westerosCardsForBottomOfDeck
    );

    if (
      _.intersection(topTwoCardsOfDeck, concatenatedForCheck).length !=
      topTwoCardsOfDeck.length
    ) {
      throw new Error(
        `Spymaster crashed due to an inconsistent deck ${westerosDeckId}!`
      );
    }

    const discardedCards = westerosDeck.filter((wc) => wc.discarded);

    const availableCards = _.without(
      westerosDeck,
      ...discardedCards.concat(topTwoCardsOfDeck)
    );

    const newWesterosDeck = _.concat(
      westerosCardsForTopOfDeck,
      availableCards,
      westerosCardsForBottomOfDeck,
      discardedCards
    );
  ...
  static deserializeFromServer(
    ...
    data: SerializedSpymasterGameState
  ): SpymasterGameState {
  ...
  deserializeChildGameState(
    data: SerializedSpymasterGameState["childGameState"]
  ): SpymasterGameState["childGameState"] {
    switch (data.type) {
      case "simple-choice":
