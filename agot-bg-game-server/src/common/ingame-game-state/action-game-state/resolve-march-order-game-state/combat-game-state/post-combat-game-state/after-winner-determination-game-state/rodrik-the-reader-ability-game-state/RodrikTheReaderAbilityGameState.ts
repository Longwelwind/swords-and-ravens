...
import SelectWesterosCardGameState, {
  SerializedSelectWesterosCardGameState,
} from "../../../../../../select-westeros-card-game-state/SelectWesterosCardGameState";
...
export default class RodrikTheReaderAbilityGameState extends GameState<
  ...
  | SelectWesterosCardGameState<RodrikTheReaderAbilityGameState>
> {
  get game(): Game {
  ...
  onSimpleChoiceGameStateEnd(choice: number): void {
    ...
    if (choice == 0) {
      ...
      this.setChildGameState(new SelectWesterosCardGameState(this)).firstStart(
        house,
  ...
  onSelectWesterosCardFinish(
    house: House,
    westerosCard: WesterosCard | null,
    deckId: number
  ): void {
    ...
    const unusedCards = shuffleInPlace(
      westerosDeck.filter((wc) => wc != westerosCard && !wc.discarded)
    );
  ...
  static deserializeFromServer(
    afterWinner: AfterWinnerDeterminationGameState["childGameState"],
    data: SerializedRodrikTheReaderAbilityGameState
  ): RodrikTheReaderAbilityGameState {
    const rodrikTheReaderAbilityGameState = new RodrikTheReaderAbilityGameState(
      afterWinner
    );

    rodrikTheReaderAbilityGameState.childGameState =
      rodrikTheReaderAbilityGameState.deserializeChildGameState(
        data.childGameState
      );
  ...
  deserializeChildGameState(
    data: SerializedRodrikTheReaderAbilityGameState["childGameState"]
  ):
    | SelectWesterosCardGameState<RodrikTheReaderAbilityGameState>
    | SimpleChoiceGameState {
    switch (data.type) {
