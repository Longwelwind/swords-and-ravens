...
> {
    ...
    @observable
    specialHouseCardModifier: { houseCard: HouseCard, combatStrength: number} | null = null;
    ...
    proceedValyrianSteelBladeUsage(): boolean {
        ...
        const commandedHouseOfVsbHolder = this.tryGetCommandedHouseInCombat(this.ingameGameState.getControllerOfHouse(valyrianSteelBladeHolder));

        // Check if the sword has not been used this round and no house card has set final combat strength of VSB holder
        if (!this.game.valyrianSteelBladeUsed && !this.isFinalCombatStrengthOverwritten(commandedHouseOfVsbHolder)) {
    ...
    isFinalCombatStrengthOverwritten(affectedHouse: House | null): boolean {
        ...
        return this.getOrderResolutionHouseCard().reduce((res, h) => {
            const houseCard = this.houseCombatDatas.get(h).houseCard;
            return houseCard?.ability ? res || houseCard.ability.doesOverwriteFinalCombatStrength(this, affectedHouse, houseCard, affectedHouseCard) : res;
        }, false);
    }
    ...
    getStatOfHouseCard(
        ...
        abilityModify: (house: House, houseCard: HouseCard, ability: HouseCardAbility, affectedHouseCard: HouseCard, baseValue: number) => number
    ): number {
        const affectedHouseCard = this.houseCombatDatas.get(affectedHouse).houseCard;
        ...
        return this.getOrderResolutionHouseCard().reduce((s, h) => {
            ...
            return houseCard.ability ? s + abilityModify(h, houseCard, houseCard.ability, affectedHouseCard, baseAmount(affectedHouseCard)) : s;
    ...
    serializeToClient(admin: boolean, player: Player | null): SerializedCombatGameState {
        ...
        return {
            ...
            specialHouseCardModifier: this.specialHouseCardModifier
                ? { houseCardId: this.specialHouseCardModifier.houseCard.id, combatStrength: this.specialHouseCardModifier.combatStrength }
                : null,
    ...
    static deserializeFromServer(resolveMarchOrderGameState: ResolveMarchOrderGameState, data: SerializedCombatGameState): CombatGameState {
        ...
        combatGameState.specialHouseCardModifier = data.specialHouseCardModifier
            ? { houseCard: combatGameState.game.getHouseCardById(data.specialHouseCardModifier.houseCardId), combatStrength: data.specialHouseCardModifier.combatStrength }
            : null;
...
export interface SerializedCombatGameState {
    ...
    specialHouseCardModifier: { houseCardId: string, combatStrength: number} | null;
