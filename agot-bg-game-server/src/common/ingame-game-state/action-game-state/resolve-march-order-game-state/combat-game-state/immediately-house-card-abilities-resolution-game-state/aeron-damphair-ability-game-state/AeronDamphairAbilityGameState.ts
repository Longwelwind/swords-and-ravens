...
export default class AeronDamphairAbilityGameState extends GameState<
  ...
  | SelectHouseCardGameState<AeronDamphairAbilityGameState>
> {
  ...
  firstStart(house: House, reduceCombatStrengthOfNewHouseCard: boolean): void {
    ...
    this.reduceCombatStrengthOfNewHouseCard =
      reduceCombatStrengthOfNewHouseCard;
  ...
  getAvailableHouseCards(house: House): HouseCard[] {
    ...
    return house.houseCards.values.filter(
      (hc) => hc != aeronDamphair && hc.state == HouseCardState.AVAILABLE,
    );
  }
  ...
  static deserializeFromServer(
    ...
    data: SerializedAeronDamphairAbilityGameState,
  ): AeronDamphairAbilityGameState {
    const aeronDamphairAbilityGameState = new AeronDamphairAbilityGameState(
      houseCardResolution,
    );

    aeronDamphairAbilityGameState.reduceCombatStrengthOfNewHouseCard =
      data.reduceCombatStrengthOfNewHouseCard;
    aeronDamphairAbilityGameState.childGameState =
      aeronDamphairAbilityGameState.deserializeChildGameState(
        data.childGameState,
      );
  ...
  deserializeChildGameState(
    data: SerializedAeronDamphairAbilityGameState["childGameState"],
  ): AeronDamphairAbilityGameState["childGameState"] {
    switch (data.type) {
      case "simple-choice":
