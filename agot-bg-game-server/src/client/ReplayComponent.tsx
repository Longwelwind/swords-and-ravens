...
export default class ReplayComponent extends Component<ReplayComponentProps> {
  ...
  renderGameStateColumn(): ReactNode {
    const roundWarning =
      this.game.maxTurns - (this.gameSnapshot?.round ?? this.game.maxTurns) ==
      1;
    const roundCritical = this.gameSnapshot?.round == this.game.maxTurns;

    const wildlingsWarning =
      this.gameSnapshot?.wildlingStrength == MAX_WILDLING_STRENGTH - 4;
    const wildlingsCritical =
      this.gameSnapshot?.wildlingStrength == MAX_WILDLING_STRENGTH ||
      this.gameSnapshot?.wildlingStrength == MAX_WILDLING_STRENGTH - 2;
    ...
    return (
      <div className={this.mapScrollbarEnabled ? "flex-ratio-container" : ""}>
        <Card
          ...
          border={border}
          style={{
            maxHeight: this.mapScrollbarEnabled ? "70%" : "none",
            borderWidth: "3px",
          }}
        >
          <Row
              ...
              <OverlayTrigger
                overlay={
                  <Tooltip id="round-tooltip">
                    <h5>
                      Round {this.gameSnapshot?.round} / {this.game.maxTurns}
                    </h5>
                  </Tooltip>
                }
                placement="auto"
          ...
          >
            ...
            <Card.Body id="game-log-panel">
              ...
              <Tab.Content className="h-100">
                ...
                {this.game.ironBank && this.gameSnapshot?.ironBank && (
