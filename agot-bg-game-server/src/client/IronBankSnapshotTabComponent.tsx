    ...
    render(): ReactNode {
        return <Col xs={12} className="h-100">
            ...
            <Row className="justify-content-center mt-4">
                {this.props.ironBank.loanSlots.slice().reverse().map((lc, i) => <Col xs="auto" key={`loan-slot-${i}`} className="pr-0">
                    <OverlayTrigger
                    ...
                    >
                        <div className="d-flex flex-row align-items-center">
                            {lc
                                ? <div className="vertical-game-card smedium hover-weak-outline" style={{ backgroundImage: lc ? `url(${loanCardImages.get(lc)})` : "none" }} />
                                : <div className="vertical-game-card game-card-slot smedium"/>}
                            <FontAwesomeIcon className="ml-2" icon={faCaretLeft} size="2x" />
                        </div>
                    </OverlayTrigger>
                    <Badge variant="secondary" style={{fontSize: "1.125rem", marginTop: "10px", marginLeft: "35px"}}>{this.ingame.game.theIronBank.loanCosts.slice().reverse()[i]}</Badge>
                </Col>)}
                <Col xs="auto">
                    <div className="vertical-game-card smedium" style={{ backgroundImage: `url(${loanCardImages.get("back")})`}} />
                </Col>
            </Row>
        </Col>;
    }
