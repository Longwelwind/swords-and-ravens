    ...
    renderGameLogData(data: GameLogData, currentRound: number, currentLogIndex: number, logTime: Date): ReactNode {
        switch (data.type) {
            ...
            case "leave-power-token-choice": {
                ...
                return <p>
                    House <b>{house.name}</b> left {data.leftPowerToken ? <>behind a</> : <b>no</b>} Power&nbsp;token in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name }</b>.
                </p>;
            }
                                        ...
                                        <ul>
                                            {recruitments.map(({ region, from, to }, i) => (
                                                <li key={"recruitment-" + region.id + "-" + i}>
                                                    {from ? (
                                                        <>
                                                            A <b>{to.name}</b> from a <b>{from.name}</b>{originatingRegion != region && (<> to <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b></>)}
                                                        </>
                                                    ) : (
                                                            <>A <b>{to.name}</b>{originatingRegion != region && (<> to <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b></>)}</>
                                                        )}
                                                </li>
                                            ))}
                                         </ul>
            ...
            case "raven-holder-replace-order": {
                ...
                return (
                    <p>
                        ...
                        </b> order in <b>{this.fogOfWar ? fogOfWarPlaceholder : orderRegion.name}</b>.
                    </p>
                );
            }
            case "raven-not-used":{
                ...
                return <>
                    ...
                    <ul>
                        {armies.map(([region, unitTypes]) => (
                            <li key={`armies-reconciled_${region.id}`}>{joinReactNodes(unitTypes.map((ut, i) => <b key={`armies-reconciled_${region.id}_${ut.id}_${i}`}>{ut.name}</b>), ", ")} in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b></li>
                        ))}
                    </ul>
                </>;
            }
            ...
            case "doran-delayed-turn": {
                ...
                return (
                    <p>
                        <b>Doran Martell</b>: House <b>{house.name}</b> delayed the turn of House <b>
                        {affectedHouse.name}</b> until the next pass of the <b>{influenceTrack}</b> track.
                    </p>
                )
            }
            ...
            case "loras-tyrell-attack-order-moved":
                ...
                return <p>
                    ...
                    to <b>{this.fogOfWar ? fogOfWarPlaceholder : embattledRegion.name}</b>.
                </p>;
            ...
            case "queen-of-thorns-order-removed": {
                ...
                return <p>
                    ...
                    a <b>{removedOrder.type.name}</b> Order of House <b>{affectedHouse.name}</b> in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b>.
                </p>;
            }
            ...
            case "cersei-lannister-order-removed": {
                ...
                return <p>
                    ...
                    of <b>{affectedHouse.name}</b> in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b>.
                </p>;
            }
            ...
            case "retreat-region-chosen": {
                ...
                return <p>
                        ...
                        {this.fogOfWar ? fogOfWarPlaceholder : regionFrom.name}</b> to <b>{this.fogOfWar ? fogOfWarPlaceholder : regionTo.name}</b>.
                </p>;
            }
            case "retreat-failed": {
                const house = this.game.houses.get(data.house);
            ...
            case "preemptive-raid-units-killed": {
                ...
                return <p>
                    ...
                    ? (<> chose to destroy {joinReactNodes(units.map(([region, unitTypes]) => <span key={`preemptive_${region.id}`}>
                        {joinReactNodes(unitTypes.map((ut, i) => <b key={`preemptive_${ut.id}_${i}`}>{ut.name}</b>), ", ")} in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b></span>), " and ")}.</>)
            ...
            case "mammoth-riders-destroy-units": {
                ...
                return <p>
                    ...
                    ? (<> chose to destroy {joinReactNodes(units.map(([region, unitTypes]) => <span key={`mammoth-riders_${region.id}`}>
                        {joinReactNodes(unitTypes.map((ut, i) => <b key={`mammoth-riders_${ut.id}_${i}`}>{ut.name}</b>), ", ")} in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b></span>), ", ")}.</>)
                    : <> had no units to destroy.</>}
                </p>;
            }
                ...
                return <p>
                    <b>The Horde Descends</b>: House <b>{house.name}</b>{units.length > 0
                        ? (<> chose to destroy {joinReactNodes(units.map(([region, unitTypes]) => <span key={`the-horde-descends_${region.id}`}>
                            {joinReactNodes(unitTypes.map((ut, i) => <b key={`the-horde-descends_${ut.id}_${i}`}>{ut.name}</b>), ", ")} in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b></span>), ", ")}.</>)
                    : <> had no units to destroy.</>}
                </p>;
            }
            case "crow-killers-knights-replaced": {
                ...
                return <p>
                    {units.length > 0
                    ? (<><b>Crow Killers</b>: House <b>{house.name}</b> replaced {joinReactNodes(units.map(([region, unitTypes], i) => <span key={`crow-killers-replace_${i}`}><b>{unitTypes.length}</b> Knight{unitTypes.length != 1 ? "s" : ""} in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b></span>), ", ")} with Footmen.</>)
            ...
            case "crow-killers-knights-killed": {
                ...
                return <p><b>Crow Killers</b>: House <b>{house.name}</b> had to destroy {joinReactNodes(units.map(([region, unitTypes], i) => <span key={`crow-killers-kill_${i}`}><b>{unitTypes.length}</b> Knight{unitTypes.length != 1 ? "s" : ""} in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b></span>), ", ")}.</p>;
            }
            ...
            case "crow-killers-footman-upgraded": {
                ...
                return <p>
                    {units.length > 0
                    ? (<><b>Crow Killers</b>: House <b>{house.name}</b> replaced {joinReactNodes(units.map(([region, unitTypes], i) => <span key={`crow-killers-upgrade_${i}`}><b>{unitTypes.length}</b> Footm{unitTypes.length == 1 ? "a" : "e"}n in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b></span>), ", ")} with Knights.</>)
            ...
            case "jon-connington-used": {
                ...
                return <p>
                    <b>Jon Conningtion</b>: House <b>{house.name}</b> chose to recruit a Knight in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b>.
                </p>;
            }
            case "bronn-used": {
                const house = this.game.houses.get(data.house);
            ...
            case "loyalty-token-placed": {
                const region = this.world.regions.get(data.region);
                return <p>
                    A loyalty&nbsp;token has been placed in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b>.
                </p>;
            }
            case "loyalty-token-gained": {
                const region = this.world.regions.get(data.region);
                return <p>
                    House <b>{this.game.targaryen?.name ?? "Targaryen"}</b> gained {data.count} Loyalty&nbsp;token{data.count != 1 ? "s" : ""} in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b>.
                </p>
            }
            case "fire-made-flesh-choice": {
                ...
                if (data.ignored) {
                    ...
                    return <p>
                        <b>Fire Made Flesh</b>: House <b>{house.name}</b> decided to destroy a dragon in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b>.
                    </p>
                    ...
                    return <p>
                        <b>Fire Made Flesh</b>: House <b>{house.name}</b> decided to place a dragon in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b>.
                    </p>
                } else {
                    return <></>;
                }
            ...
            case "playing-with-fire-choice": {
                ...
                return <p>
                    <b>Playing with Fire</b>: House <b>{house.name}</b> chose to place a <b>{unitType.name}</b> in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b>.
                </p>
            }
            ...
            case "move-loyalty-token-choice": {
                ...
                if (data.powerTokensDiscardedToCancelMovement == 0) {
                    ...
                    return <p>
                        House <b>{house.name}</b> moved a loyalty&nbsp;token from <b>{this.fogOfWar ? fogOfWarPlaceholder : regionFrom.name}</b> to <b>{this.fogOfWar ? fogOfWarPlaceholder : regionTo.name}</b>.
                    </p>
                } else {
                    return <></>;
                }
                ...
                return <>
                    <p>
                        House <b>{house.name}</b> resolved an <b>Iron&nbsp;Bank</b> order in <b>
                            {this.fogOfWar ? fogOfWarPlaceholder : region.name}</b> and paid <b>{data.paid}</b> Power&nbsp;token{data.paid != 1 ? "s" : ""} for
                    </p>
                    <div style={{ display: 'flex', justifyContent: 'center' }}>
            ...
            case "order-removed": {
                ...
                return house ?
                    <p>
                        House <b>{house.name}</b> removed their <b>{orderType.name}</b> order in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b>.
                    </p>
                    :
                    <p>
                        A <b>{orderType.name}</b> order has been removed from <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b>.
                    </p>
            }
            case "interest-paid": {
                const house = this.game.houses.get(data.house);
            ...
            case "debt-paid": {
                ...
                return <p>
                    ...
                    ? (<>House <b>{resolver.name}</b> chose to destroy {joinReactNodes(units.map(([region, unitTypes]) =>
                            <span key={`pay-debt_${region.id}_${house.id}`}>{joinReactNodes(unitTypes.map((ut, i) => <b key={`pay-debt_${ut.id}_${i}`}>{ut.name}</b>), ", ")} in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b></span>), ", ")} of House <b>{house.name}</b>.</>)
                ...
                return <p>
                    <b>{loan.name}</b>: House <b>{house.name}</b>{placedSellswords.length > 0 ? (<> chose to
                        place {joinReactNodes(placedSellswords.map(([region, unitTypes]) => <span key={`place-sellsword_${region.id}`}>
                            {joinReactNodes(unitTypes.map((ut, i) => <b key={`place-sellsword_${region.id}_${ut.id}_${i}`}>{ut.name}</b>), ", ")} in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b></span>), " and ")}.</>)
            ...
            case "pyromancer-executed": {
                ...
                const region = this.world.regions.get(data.region);
                return <p>
                    <b>Pyromancer</b>: House <b>{house.name}</b> chose to degrade <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b> and place a <b>{data.upgradeType}</b> there.
                </p>;
            }
            case "expert-artificer-executed": {
                ...
                const region = this.world.regions.get(data.region);
                return <p>
                    <b>Expert Artificer</b>: House <b>{house.name}</b> chose to place a <b>Crown</b> in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b> and gained <b>{data.gainedPowerTokens}</b> Power token{data.gainedPowerTokens != 1 ? "s" : ""}.
                </p>;
            }
            case "loyal-maester-executed": {
                ...
                return <p>
                    <b>Loyal Maester</b>: House <b>{house.name}</b> chose to place a <b>Barrel</b> in {joinReactNodes(regions.map(r => <b key={`loyal_maester_${r.id}`}>{this.fogOfWar ? fogOfWarPlaceholder : r.name}</b>), ' and in ')}.
                </p>;
            }
            case "master-at-arms-executed": {
                ...
                const regions = data.regions.map(rid => this.world.regions.get(rid));
                return <p>
                    <b>Master-at-Arms</b>: House <b>{house.name}</b> chose to upgrade the castles in {joinReactNodes(regions.map(r => <b key={`master-at-arms_${r.id}`}>{this.fogOfWar ? fogOfWarPlaceholder : r.name}</b>), ' and in ')}.
                </p>;
            }
            case "savvy-steward-executed": {
                ...
                const region = this.world.regions.get(data.region);
                return <p>
                    <b>Savvy Steward</b>: House <b>{house.name}</b> chose to place a <b>Barrel</b> in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b>. Their new supply level is <b>{data.newSupply}</b>.
                </p>;
            }
            ...
            case "garrison-removed": {
                ...
                return <p>
                    A <b>garrison token</b> with strength <b>{data.strength}</b> was <b>removed</b> from <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b>.
                </p>;
            }
            case "garrison-returned": {
                ...
                return <p>
                    A <b>garrison token</b> with strength <b>{data.strength}</b> was <b>returned</b> to <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b>.
                </p>;
            }
            ...
            case "mace-tyrell-asos-order-placed": {
                ...
                return <p>
                    <b>Mace Tyrell</b>: House <b>{house.name}</b> decided to place a <b>{order.type.name}</b> order in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b>.
                </p>;
            }
            ...
            case "control-power-token-removed": {
                ...
                return <p>
                    A <b>Power token</b> of House <b>{house.name}</b> was removed from <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b>.
                </p>;
            }
            case "game-paused": {
                return <p>
            ...
            case "support-attack-against-neutral-force": {
                ...
                return <p>
                    House <b>{supporter.name}</b> {data.refused
                        ...
                        : <>would support House <b>{house.name}</b></>} to attack the Neutral Force in <b>{this.fogOfWar ? fogOfWarPlaceholder : region.name}</b>.
                </p>;
            }
            case "houses-swapped": {
