...
def enrich_games(request, games: List[Game],
    ...
    for game in games:
        ...
        # Check whether there are unseen private messages
        if enrich_important_messages and game.player_in_game is not None\
            and len(game.player_in_game) == 1 and "important_chat_rooms" in game.player_in_game[0].data:
            # `important_chat_rooms` will contain a list of room ids that must trigger a warning
            # if there are unseen messages.
            important_chat_rooms = game.player_in_game[0].data["important_chat_rooms"]
            ...
            unread_messages = UserInRoom.objects.filter(
                ...
                user=game.player_in_game[0].user,
        ...
        # Check whether there are unseen public messages
        if enrich_important_messages and game.player_in_game is not None\
            and len(game.player_in_game) == 1 and "publicChatRoomId" in game.view_of_game:
            # `important_chat_rooms` will contain a list of room ids that must trigger a warning
            # if there are unseen messages.
            public_room_id = game.view_of_game["publicChatRoomId"]

            # This is a query inside a loop, not super good for performance,
            # but since this only applies to games of the player, it should not impact performance that much.
            unread_public_messages = UserInRoom.objects.filter(
                Q(room__messages__created_at__gt=F("last_viewed_message__created_at"))
                | Q(last_viewed_message__isnull=True),
                user=game.player_in_game[0].user,
